...
class AbstractDataLoader(abc.ABC):
    ...
    @abc.abstractmethod
    def get_rgb_chips_from_geom(self, geom, buffer, show_outline=True):
    ...
    def get_data_stack_from_geom(self, geom, buffer):
        """Returns a time-series stack of data images corresponding to a query geometry. While `get_rgb_chips_from_geom(.)` returns just the RGB component of the imagery, this method should return
    ...
    def data_stack_to_rgb(self, images):
        """A convenience method that converts the `images` that are returned by `get_data_stack_from_geom(.)` to `rgb_images` (i.e. the kind returned by `get_rgb_chips_from_geom`).
        ...
        Returns:
            rgb_images: A list of RGB image patches (with `np.uint8` dtypes), one for each patch in `images`. These should be processed in the same way that `get_rgb_chips_from_geom(.)` processes the source imagery.
        """
        raise NotImplementedError()
...
class PlanetaryComputerS2DataLoader(AbstractDataLoader):
    ...
    def __init__(self, geoms, pc_subscription_key, search_start="2015-01-01", search_end="2020-12-31"):
    ...
    def get_rgb_chips_from_geom(self, geom_idx, buffer, show_outline=True):
        ...
        for item in items:
            ...
            with rasterio.Env(**RASTERIO_BEST_PRACTICES):
                with rasterio.open(signed_href) as f:
                    ...
                    is_all_zero = np.all((full_image == 0))
                    if is_all_zero:
                        continue
